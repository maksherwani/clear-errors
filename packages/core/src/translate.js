 ...
 */
async function translateError(error, options = {}) {
    const { customRules = [], useAI = false, aiApiKey, locale = 'en', tone = 'friendly', } = options;
    // Combine custom rules with default rules (custom rules have higher priority)
    const allRules = [...customRules, ...rules_1.DEFAULT_RULES];
    // Try to find a matching rule
    const matchingRule = (0, rules_1.findMatchingRule)(error, allRules);
    if (matchingRule) {
 ...
 */
async function translateWithAI(error, apiKey, locale, tone) {
 ...
 */
function adjustTone(message, tone) {
 ...
 */
function generateFallbackMessage(error, tone) {
